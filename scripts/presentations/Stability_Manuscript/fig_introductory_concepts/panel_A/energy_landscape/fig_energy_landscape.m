function out = fig_energy_landscape(cfg)
% FIG_ENERGY_LANDSCAPE Figure 1 panel A (top): the effective potential.
%
%   out = FIG_ENERGY_LANDSCAPE()
%   out = FIG_ENERGY_LANDSCAPE('gammas', [0.9 1.6 2.5])
%
% The 1-D reduction of the network, at the same three gains as the traces and
% eigenspectra below it. For a single self-coupled unit with a tanh nonlinearity
% the dynamics are a gradient flow:
%
%     dx/dt = -x + gamma*tanh(x) = -dU/dx,
%     U(x)  = x^2/2 - gamma*log(cosh(x)).
%
% The curvature at the origin is U''(0) = 1 - gamma, so x = 0 is a MINIMUM
% (stable) for gamma < 1 and a MAXIMUM (unstable) for gamma > 1 -- the same
% transition the eigenspectra show as the disk crossing Re = 0, and the traces
% show as relaxation against chaos.
%
% Arrows show the flow direction (always downhill). The ball is drawn at x = 0
% in every panel -- the same fixed point the eigenspectrum linearizes about --
% so the story is told by the landscape changing underneath it, not by the ball
% moving.
%
% ANALYTIC: no model is constructed. It takes no preset, and it is the one part
% of panel A that cannot: it is the 1-D reduction, not the network. Its `gammas`
% MUST match fig_introductory_concepts, which is why they are an argument rather
% than a literal in both -- the master script passes one value to both.
%
...
arguments
    ...
    cfg.preset_name (1,:) char    = ''   % unused; analytic, see above
end
...
gammas      = cfg.gammas;   % MUST match fig_introductory_concepts (same panel)
...
%% --- Save -------------------------------------------------------------------
fig_tag = 'panelA_energy_landscape';
...
if cfg.save
    ...
    write_figure_readme(out_dir, struct( ...
        'tag',    'fig_energy_landscape', ...
        'title',  'Stability_Manuscript figure 1 panel A (top): effective potential', ...
        'script', 'fig_energy_landscape.m', ...
        'what',   ['The effective potential U(x) = x^2/2 - gamma*log(cosh(x)) ' ...
                   'for the 1-D reduction of the network, at three gains. The ' ...
                   'curvature at the origin is 1 - gamma, so x = 0 is a stable ' ...
                   'minimum below gamma = 1 and an unstable maximum above it. ' ...
                   'Arrows show the flow direction, always downhill; the ball ' ...
                   'sits at x = 0 in every panel, so the story is the landscape ' ...
                   'changing underneath it.'], ...
        'how',    ['ANALYTIC -- no model is constructed. This is the one part ' ...
                   'of panel A that takes no preset, because it is the 1-D ' ...
                   'reduction rather than the network.'], ...
        'settings', struct('gammas', cfg.gammas), ...
        'figures', {out.files}, ...
        'notes',  ['THE GAMMAS MUST MATCH fig_introductory_concepts, which draws ' ...
                   'the traces and eigenspectra directly below this panel at ' ...
                   'the same gains. They are an argument in both functions ' ...
                   'rather than a literal in each, so the master script can ' ...
                   'pass one value to both and they cannot drift apart.']));
end
end
