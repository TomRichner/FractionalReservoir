...
for k = 1:n_cases
    ...
    % One cell type, so plot_data.x has exactly one field and it is the whole
    % network. Read it by name off the preset rather than assuming 'all'.
    results(k).x     = model.plot_data.x.(model.cell_type_names{1});   % N x nt
    results(k).LLE   = LLE;
    results(k).R     = model.R;
...
if cfg.save
    ...
    write_figure_readme(out_dir, struct( ...
        ...
        'figures', {out.files}, ...
        'sections', struct( ...
            'heading', {'measured exponents', 'one cell type'}, ...
            'body',    {lle_note, readme_one_type()})));
end
end

%% ------------------------------------------------------------------------
function s = readme_one_type()
s = ['ONE CELL TYPE. This is a single undifferentiated population: zero-mean ' ...
     'Gaussian weights taking both signs, so it is Dale-free and no E/I split ' ...
     'exists to name. The type is called ''all'' for that reason. Until ' ...
     '2026-08-23 the preset used TWO statistically identical types named A and ' ...
     'B, purely because SRNNCellTypePairs could not build a one-type model ' ...
     '(build_network configured its RMTBlocks generator piecemeal where ' ...
     'set_types is required to change the number of types). That is fixed, and ' ...
     'the weight matrix is BIT-IDENTICAL across the change: the two types had ' ...
     'identical zero-mean statistics, so the per-block scaling was uniform and ' ...
     'the underlying draw never depended on how it was partitioned. This ' ...
     'figure is therefore unchanged, measured exponents included. The spectral ' ...
     'radius is R = gamma exactly either way.'];
end
