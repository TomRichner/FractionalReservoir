...
% Fig_param_space_allStd2.m
% Stability_Manuscript presentation figure: parameter-space distributions, for the
% SRNNCellTypePairs "allStd2" runs. Recreates the param-space histogram figures
...
%
% Identical to ../fig_param_space_allStd/Fig_param_space_allStd.m except for
% data_root and the output names: this one targets the OLDER medium run
% run_all_aug_14_26_00_48 (preset celltype_pairs_uniform_std_n500_mu5p5_nodrive_sig1p5,
% S_c = 0, no added noise, commit 9cb280e, 4 grid levels), so the two can be
% compared.
%
...
% param-space LLEs span roughly -10 to +4.7, so the outermost bins carry a large
% share of the distribution.
%
% See also: Fig_sensitivity_analysis_allStd2, Fig_sfa_EOC_allStd2
...
if isempty(lle_fig) || isempty(mr_fig)
    error('Fig_param_space_allStd2:MissingFigure', ...
...
fig_tag = 'Fig_ParamSpace_allStd2';
...
desc_path = fullfile(out_dir, 'README_fig_param_space_allStd2.txt');
fid = fopen(desc_path, 'w');
cleanup = onCleanup(@() fclose(fid));

fprintf(fid, 'Stability_Manuscript figure: Parameter-space distributions (allStd2)\n');
fprintf(fid, '==================================================================\n\n');
...
fprintf(fid, '  preset the param-space LLEs span roughly -10 to +4.7, so the outermost\n');
...
% cleaned up.
    ...
    for r = 1:size(renames, 1)
        ...
        if isfile(src)
            ...
            warning('Fig_param_space_allStd2:ExportMissing', ...
