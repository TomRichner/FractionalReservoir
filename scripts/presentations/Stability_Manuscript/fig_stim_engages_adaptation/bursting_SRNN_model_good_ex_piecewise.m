...
%  level_of_chaos. The tilde params are given in "F units" -- multiples of the
%  normalization factor F = 1/sqrt(N*alpha*(2-alpha)), so that R=1 when all four
%  are equal to 1. The model computes F itself from n & indegree.
mu_E_tilde_relative    =  3;   % normalized E mean      (class default  3)
mu_I_tilde_relative    = -2;   % normalized I mean      (class default -4)
sigma_E_tilde_relative =  1;   % normalized E std dev   (class default  1)
sigma_I_tilde_relative =  1;   % normalized I std dev   (class default  1)
E_W_relative           =  0;   % common mean offset added to both mu tildes (default 0)
...
% The model builds the nonlinearity itself from `activation` + S_a/S_c, so the
% three named ones need no handles here. relu is not one of them, so it goes
% through the activation_custom escape hatch.
switch lower(nonlinearity)
    case {'piecewise', 'logistic', 'tanh'}
        activation_args = {'activation', lower(nonlinearity)};
    case 'relu'
        activation_args = {'activation_custom', ...
            {@(x) max(0, x), @(x) double(x > 0)}};
    otherwise
...
model = SRNNModel2( ...
    ...
    'mu_E_tilde_relative', mu_E_tilde_relative, 'mu_I_tilde_relative', mu_I_tilde_relative, ...
    'sigma_E_tilde_relative', sigma_E_tilde_relative, 'sigma_I_tilde_relative', sigma_I_tilde_relative, ...
    'E_W_relative', E_W_relative, 'zrs_mode', zrs_mode, ...
