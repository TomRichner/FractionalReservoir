...
fprintf(fid, '$$\ns_i = %s\n$$\n\n', strjoin(parts, ' \\, '));
if M.n_b_max > 1
    fprintf(fid, ['Note the depression timescales enter as a **product**, so ' ...
        '%d of them square the depression at steady state -- there is no ' ...
        'split of $\\tau$ that would make them match a single timescale. ' ...
        'Adaptation, by contrast, enters as a **sum**, so splitting $c$ as a ' ...
        'budget over its timescales leaves the steady state unchanged.\n\n'], ...
        M.n_b_max);
end
