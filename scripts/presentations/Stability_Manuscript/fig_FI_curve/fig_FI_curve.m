...
if cfg.save
    ...
    write_figure_readme(out_dir, struct( ...
        'tag',    'fig_FI_curve', ...
        'title',  'Stability_Manuscript figure: SFA and STD reshape the F-I curve', ...
        'script', 'fig_FI_curve.m', ...
        'what',   {{sprintf(['Conceptual, ANALYTIC figure (no simulation): the synaptic ' ...
                    'output br = b * phi(x - c*a) against input x, with ' ...
                    'phi(x) = 1/(1+exp(-4*(x - S_c))) and S_c = %.2f.'], S_c), ...
                   ['Left panel (SFA): b = 1, a swept 0 -> 1. Subtracting c*a inside ' ...
                    'phi shifts the curve HORIZONTALLY, raising the threshold.'], ...
                   ['Right panel (STD): a = 0, b swept 1 -> 0. The b prefactor ' ...
                    'rescales the curve VERTICALLY, reducing gain and the ' ...
                    'saturation ceiling.']}}, ...
        'settings', struct('S_c', S_c, 'c_concept', c_concept, ...
                           'a_levels', a_levels, 'b_levels', b_levels), ...
        'figures', {out.files}, ...
        'notes',  {{sprintf(['c = %.2f is ENLARGED for visibility; the model default ' ...
                    'c_E = 0.5/3 is too small to see. The functional form ' ...
                    'b*phi(x - c*a) is exact -- only the magnitude of c is ' ...
                    'chosen for clarity.'], c_concept), ...
                   ['This figure is deliberately NOT built from the paper''s preset. ' ...
                    'It is a schematic of the form of the equations, not a picture ' ...
                    'of a particular network, so it keeps the logistic phi at ' ...
                    'S_c = 0.4 rather than the preset''s piecewise nonlinearity.']}}));
end
end
