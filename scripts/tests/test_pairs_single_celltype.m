...
%
% See also: SRNNCellTypePairs, RMTBlocks/set_types, srnn_param_preset
...
fprintf('\n-- D = 2 invariance (frozen pre-change checksums) --\n');
% Cut from 15 presets to 4 when the exploratory celltype_pairs_* family was
% retired by the c/K change (2026-08-25). The retired presets now throw, so they
% cannot be built to check anything; their W values are unchanged in git at
% cbcf637. The surviving four keep their ORIGINAL frozen numbers -- the
% connectivity was never touched by either refactor, which is exactly what this
% table is here to keep true.
frozen = { ...
    'celltype_pairs_Sc0p2_noise0p025_dualStd_4cond',                 -1.14156058489,  29.7665779012; ...
...
% This used to exercise srnn_adaptation_conditions' n_cell_types argument. That
% function is gone: presets now state their conditions in full, so the row length
% is a property of each PRESET rather than of an argument to a shared builder.
% The C = 1 side is checked in the next block, over the three C = 1 presets; this
% is the C = 2 complement.
...
for nm = {'sompolinsky_pairs', 'single_neuron_stf', 'single_neuron_dualStd'}
    ...
    ok = strcmp(cls, 'SRNNCellTypePairs') && d.n_cellTypes == 1 && ...
         all(cellfun(@(c) isscalar(c.tau_a),cond));
...
paper = srnn_param_preset('celltype_pairs_Sc0p2_noise0p025_dualStd_4cond');
...
[~, ~, paper_cond] = srnn_param_preset('celltype_pairs_Sc0p2_noise0p025_dualStd_4cond');
...
m = SRNNCellTypePairs('n_cellTypes', 1, 'cell_type_names', {'all'}, 'f', 1, ...
    ...
    'c', 0, 'tau_a', {[]}, 'synapse_config', struct(), 'x0_std', 1.0, ...
...
m = SRNNCellTypePairs('n_cellTypes', 1, 'cell_type_names', {'E'}, 'f', 1, ...
    ...
    'c', 0.5, 'tau_a', {log_ladder(0.25, 10, 3)}, 'synapse_config', sc, 'x0_std', 0, ...
