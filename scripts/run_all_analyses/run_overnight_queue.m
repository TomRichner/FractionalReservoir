...
%
% Both entries are the same preset at two fidelities: fast2 first as a cheap
% check that the operating point behaves, then medium for the real numbers. The
% integrator follows from the preset's noise automatically -- sra1 here, chosen
% by analysis_run_config rather than named.
queue = {
    struct('label', 'Sc0.2 noise0.025 (fast2)', ...
           'preset', 'celltype_pairs_Sc0p2_noise0p025', ...
           'run_mode', 'fast2')
    struct('label', 'Sc0.2 noise0.025 (medium)', ...
           'preset', 'celltype_pairs_Sc0p2_noise0p025', ...
           'run_mode', 'medium')
    };
