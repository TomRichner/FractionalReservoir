function cfg = mu7revisedMedium_config()
% MU7REVISEDMEDIUM_CONFIG The mu7revised network at run_mode MEDIUM: the manuscript-statistics run (TR, 2026-09-16).
% mu7revisedAgain with run_mode fixed to medium (11 levels x 15 reps per 1-D
% sweep, 128 joint points at 400 Hz, 15 MC trials, K = 200 spectrum, numerics
% at 25 LLE trials). Same physics as mu7revised on its own four preset copies.
...
cfg.preset_name = 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7revisedMedium';
run_mode = 'medium';   % FIXED: this bundle IS the medium run (TR, 2026-09-16)
cfg.run_mode    = run_mode;

%% Where things land -- both fixed, so this cannot touch any other run
cfg.run_dir  = 'data/mu7revisedMedium';   % analyses write, figures read
cfg.fig_root = 'figs/mu7revisedMedium';   % overwritten in place
...
cfg.mc_preset            = 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0_dualStd_3cond_mu7revisedMedium';
...
cfg.local_lyapunov_preset = 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_steps5s_noise0p025_dualStd_3cond_mu7revisedMedium';
...
cfg.single_neuron_preset = 'single_neuron_mu7revisedMedium';
...
cfg.illustration_display_window = [0 30];
% The step occupies the MIDDLE THIRD of the display window, [10 20] s, and
% 25 neurons of each type are saved (50 drawn); the mu7revised bundle's step
% started at 15 s and stayed on, with 4 per type saved (TR, 2026-09-15).
cfg.illustration_step_window = [10 20];
cfg.illustration_neurons_per_type = 25;
