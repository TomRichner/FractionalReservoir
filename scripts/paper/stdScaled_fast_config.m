function cfg = stdScaled_fast_config()
% STDSCALED_FAST_CONFIG The STD strength-matched (scaled) network at 'fast', every analysis.
%
%   run_dir = run_all_paper_analyses(stdScaled_fast_config());
%   results = make_all_paper_figures(stdScaled_fast_config());
%
...
%
% RERUNNING: delete data/stdscaled_fast first.
% run_all_paper_analyses refuses a run directory that is not absent or empty.
%
% See also: stdScaled_fast_run, run_all_paper_analyses,
...
cfg.preset_name = 'celltype_pairs_sfaEI_Sc0p2sig0p1_noise0p025_dualStdScaled_3cond_mu8p25';
cfg.run_mode    = 'fast';

%% Where things land -- both fixed, so this cannot touch any other run
cfg.run_dir  = 'data/stdscaled_fast';   % analyses write, figures read
cfg.fig_root = 'figs/stdscaled_fast';   % overwritten in place
