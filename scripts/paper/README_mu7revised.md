...
| 3 | Native saved-data eigenvalue occupancy and simplified switching-input traces. Input is eight fixed actual neurons. Local positive QR sum is `sum(max(local_rates,0),2)/log(2)`; percentage uses the same samples and strict positivity over the saved second-half window. This measures positive local growth across tracked QR directions; it is not KS entropy. |
...
New labels/axes use 14-point fonts, with 16-point single-line column titles, one E/I legend, row letters A–F and gray column dividers. The 10-second scale bar occupies the lower-left lambda panel at x = 5…15, y = -4.8. Per-neuron colors cannot be exactly reassigned from flattened antialiased raster overlaps; the current E/I shades are retained. The future numeric plotter preserves neuron ordering across rows and draws E over I; the later trace-density update below widens the reddish/bluish palette spread.
...
Four native rows now show eigenvalue density, 24 actual fixed input-neuron traces (12 E and 12 I), local and accumulated leading exponents, and local positive QR-rate sum. Condition headings use the standard black/gold/blue colors and 16-point type; all other fonts are 14. Figure 2 headings now use those condition colors too. Rows 3–4 share [0,60] s axes. The current source is **K=30**, accumulation **[30,60] s**, with saved segment-start timestamps 30…59.95 s. The figure explicitly states this; requested K=50 and early accumulation cannot be recovered by replotting.

The future bundle explicitly sets `local_lyapunov_K=50`, `local_lyapunov_T=40`, `local_lyapunov_step_duration_s=5` even in fast mode, `local_lyapunov_accumulation_start_s=1`, `local_lyapunov_warmup_s=15`, and `local_lyapunov_simulation_start_s=-15`. This provides the full alignment [-14,1] s before accumulation [1,40] s. Saved QR times retain the stage's segment-start convention (first completed [1,1.05] segment is indexed at 1). Legacy callers retain K by mode and [T/2,T] accumulation. No new switching simulation was run locally. The local positive sum describes expansive transients across tracked QR directions and is not called KS entropy.

### Figure 2 trace-density and label follow-up

Current Figure 2 now adds descriptive LaTeX row labels and a green dashed zero reference (0.5-point width) in each lambda panel. Its calibrated raster retains the original neuron count, per-neuron colors and curve widths: individual overlapping traces cannot be faithfully removed, recolored or thinned from this source. No matching numeric trajectory archive was found and no new simulation was run.

For future numeric representative data, plotting selects half of the saved neurons per type: the existing four-per-type saved schema displays two (indices 1 and 4), consistently across state rows. Neuron and local-rate lines use width0.8; finite-LLE lines use width1.25. E shades use distinct dark red/red/coral/rose; I shades use navy/blue/cyan/teal without nearly invisible pale colors. Saved data themselves are unchanged. Figures 2 and 3 now spell out No adaptation, Single-timescale adaptation, and Multiple-timescale adaptation, with raised single-line16-point headings and the existing condition colors.

## Latest compact-canvas and local-rate update

Figure1 uses910×665 pixels; Figure4 uses910×808 (pixel-rounded); Figure5 uses1295×406. These are fixed reductions of the creation baselines, keeping all14-point fonts unchanged. Figure4 colors sensitivity legend text, anchors(A) immediately above subplot1, sets row3 x ticks0,0.5,1 and all axes linewidth1. Figure5 retains its shared legend centered below subplot3.

Figure3 now restores the actual saved local leading rate alongside the accumulated estimate. Requested fixed y limits clip some local excursions; no samples are removed or changed. Row D is labeled 'Expansive transients' / 'top-K sum, bits/s', with actual K=30 for current data and K=50 for the new run. It sums positive local QR growth rates and divides by ln(2); it does not measure KS entropy or information transmission. The future bundle supersedes the earlier K100/60-second request with K50 over40 seconds and8 input intervals at5-second cadence; the generic sampled staircase changes within one400-Hz sample of nominal boundaries. Negative simulation start-15 provides alignment[-14,1]. Future model/input construction was checked without calling run. Current data remain K30/60seconds, accumulated over30–60seconds.

## mu7revisedAgain (2026-09-15 evening)

`mu7revisedAgain_config` / `mu7revisedAgain_run` are mu7revised with the illustration protocol corrected (TR): the uniform input step of the dynamics figure occupies the **middle third** of the displayed `[0,30]` s window (`cfg.illustration_step_window = [10 20]`, via `paper_midpoint_input`'s new optional `step_off`) instead of starting at 15 s and staying on, and `cfg.illustration_neurons_per_type = 25` (the grouped Figure 2 draws half of them, 12 per type, in `SRNNModel2`'s E/I palettes; the input step is row (A), amplitude 0.25, and the local rate in row (G) is low-pass filtered at 2 Hz for display; TR 2026-09-16). The bundle has its own four preset copies (`..._mu7revisedAgain`, `single_neuron_mu7revisedAgain`), physically identical to mu7revised's. Grouped Figure 5's gain panels show only the first 2 s after the perturbation (ticks 0, 1, 2 s) whatever horizon was saved. The `paper_illustrations` stage itself was fixed the same day (it could not grow its results struct, so it had never run) and `fig_main8_psd` now reads the run's own `bursting_psd.fig` instead of a checksummed archive PNG. `data/mu7revised_fast` had its `representative_dynamics` / `single_neuron` outputs regenerated with the corrected protocol (the settings saved in each `.mat` record the step window and neuron count).
