...
Figure3 now restores the actual saved local leading rate alongside the accumulated estimate. Requested fixed y limits clip some local excursions; no samples are removed or changed. RowD's two-line KS label uses actual K=30 for current data, and denotes the finite-K sum of positive local QR rates in bits/s, a provisional entropy proxy. The future bundle supersedes the earlier K100/60-second request with K50 over40 seconds and8 input intervals at5-second cadence; the generic sampled staircase changes within one400-Hz sample of nominal boundaries. Negative simulation start-15 provides alignment[-14,1]. Future model/input construction was checked without calling run. Current data remain K30/60seconds, accumulated over30–60seconds.
