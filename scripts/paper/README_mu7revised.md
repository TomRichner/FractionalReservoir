...
New labels/axes use 14-point fonts, with 20-point column titles, one E/I legend, row letters A–F and gray column dividers. The 10-second scale bar occupies the lower-left lambda panel at x = 5…15, y = -4.8. Per-neuron colors cannot be exactly reassigned from flattened antialiased raster overlaps; the current E/I shades are retained. The future numeric plotter preserves neuron ordering across rows and draws E over I; the later trace-density update below widens the reddish/bluish palette spread.
...
For future numeric representative data, plotting selects half of the saved neurons per type: the existing four-per-type saved schema displays two (indices 1 and 4), consistently across state rows. Neuron and local-rate lines use width 0.5; finite-LLE lines use width 1.0. E shades span dark red to light coral; I shades span dark blue to light cyan. Saved data themselves are unchanged. Figures 2 and 3 now spell out No adaptation, Single-timescale adaptation, and Multiple-timescale adaptation, with wrapped headings and the existing condition colors.
