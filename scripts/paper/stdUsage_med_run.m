% STDUSAGE_MED_RUN STD strength-matched (usage) network, medium, self-contained.
%
%   Open this file and press Run. setup_paths is called on the first line.
%
% Every setting comes from stdUsage_med_config(), which states
...
%   run_mode  medium
%   run_dir   data/stdusage_med
%   fig_root  figs/stdusage_med
%
...
%
% RERUNNING REQUIRES DELETING data/stdusage_med FIRST.
% run_all_paper_analyses refuses a run directory that is not absent or empty.
%
% See also: stdUsage_med_config, run_all_paper_analyses,
%           make_all_paper_figures

setup_paths();

cfg = stdUsage_med_config();

run_dir = run_all_paper_analyses(cfg);
results = make_all_paper_figures(cfg);

fprintf('\n========================================================\n');
fprintf('SINGLE vs MULTI TIMESCALE (STD usage, medium) RUN COMPLETE\n');
