function cfg = sfaEI_tauSpread0p25_fast_config()
% SFAEI_TAUSPREAD0P25_FAST_CONFIG The paper network with per-neuron SFA ladders (spread 0.25) at 'fast', every analysis, MC noise-free.
%
%   run_dir = run_all_paper_analyses(sfaEI_tauSpread0p25_fast_config());
%   results = make_all_paper_figures(sfaEI_tauSpread0p25_fast_config());
%
...
%
% RERUNNING: delete data/sfaEI_tauSpread0p25_fast first.
% run_all_paper_analyses refuses a run directory that is not absent or empty.
%
% See also: sfaEI_tauSpread0p25_fast_run, run_all_paper_analyses,
...
cfg.preset_name = 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noise0p025_dualStd_3cond_mu8p25';
cfg.run_mode    = 'fast';

%% Where things land -- both fixed, so this cannot touch any other run
cfg.run_dir  = 'data/sfaEI_tauSpread0p25_fast';   % analyses write, figures read
cfg.fig_root = 'figs/sfaEI_tauSpread0p25_fast';   % overwritten in place
...
cfg.mc_preset            = 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noise0_dualStd_3cond_mu8p25';
