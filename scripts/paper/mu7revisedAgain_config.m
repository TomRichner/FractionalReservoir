function cfg = mu7revisedAgain_config(run_mode)
% MU7REVISEDAGAIN_CONFIG Independent mu=7 bundle: mu7revised with the illustration protocol corrected.
% Same physics as mu7revised (its own preset copies, per the bundle rule);
% the input step of the dynamics illustration occupies the middle third of
% the display window and 25 neurons per type are saved (TR, 2026-09-15).
% Defaults to FAST. Use mu7revisedAgain_config('medium') for manuscript estimates.
...
cfg.preset_name = 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7revisedAgain';
cfg.run_mode    = run_mode;

%% Where things land -- both fixed, so this cannot touch any other run
cfg.run_dir  = ['data/mu7revisedAgain_' run_mode];   % analyses write, figures read
cfg.fig_root = ['figs/mu7revisedAgain_' run_mode];   % overwritten in place
...
cfg.mc_preset            = 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0_dualStd_3cond_mu7revisedAgain';
...
cfg.local_lyapunov_preset = 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_steps5s_noise0p025_dualStd_3cond_mu7revisedAgain';
