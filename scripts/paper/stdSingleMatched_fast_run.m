% STDUSAGE_FAST_RUN STD strength-matched (single-strengthened) network, fast, self-contained.
%
%   Open this file and press Run. setup_paths is called on the first line.
%
% Every setting comes from stdSingleMatched_fast_config(), which states
% ALL of them itself -- it does not call paper_config. Read that one file and
% you know the whole run.
%
%   preset    celltype_pairs_sfaEI_Sc0p2sig0p1_noise0p025_dualStdSingleMatched_3cond_mu8p25
%             (mu x1.5, SFA on E AND I, per-neuron S_c with sigma 0.1)
%   run_mode  fast
%   run_dir   data/stdsinglematched_fast
%   fig_root  figs/stdsinglematched_fast
%
...
%
% RERUNNING REQUIRES DELETING data/stdsinglematched_fast FIRST.
% run_all_paper_analyses refuses a run directory that is not absent or empty.
%
% See also: stdSingleMatched_fast_config, run_all_paper_analyses,
%           make_all_paper_figures

setup_paths();

cfg = stdSingleMatched_fast_config();

run_dir = run_all_paper_analyses(cfg);
results = make_all_paper_figures(cfg);

fprintf('\n========================================================\n');
fprintf('SINGLE vs MULTI TIMESCALE (STD single-matched, fast) RUN COMPLETE\n');
