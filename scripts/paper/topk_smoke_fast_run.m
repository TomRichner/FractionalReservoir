% TOPK_SMOKE_FAST_RUN Single vs multi timescale, self-contained.
%
%   Open this file and press Run. setup_paths is called on the first line.
%
% Every setting comes from topk_smoke_fast_config(), which states
...
%   run_mode  medium
%   run_dir   data/topk_smoke_fast
%   fig_root  figs/topk_smoke_fast
%
...
%
% RERUNNING REQUIRES DELETING data/topk_smoke_fast FIRST.
% run_all_paper_analyses refuses a run directory that is not absent or empty.
%
% See also: topk_smoke_fast_config, run_all_paper_analyses,
%           make_all_paper_figures

setup_paths();

cfg = topk_smoke_fast_config();

run_dir = run_all_paper_analyses(cfg);
results = make_all_paper_figures(cfg);

fprintf('\n========================================================\n');
fprintf('SINGLE vs MULTI TIMESCALE (top-K smoke, fast) RUN COMPLETE\n');
