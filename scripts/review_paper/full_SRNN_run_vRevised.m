function [t_out, S_out, params, lya_results, plot_data] = full_SRNN_run_vRevised(u_ex_scale, n_a_E, n_b_E, level_of_chaos, rng_seeds, save_dir, save_figs, save_workspace, note, time_config)
...
params.tau_a_E = logspace(log10(0.5), log10(10), params.n_a_E);  % Logarithmically spaced from 0.1 to 10
params.tau_a_I = logspace(log10(0.5), log10(10), params.n_a_I);  % Logarithmically spaced from 0.1 to 10
params.c_E = 0.15/3;  % Adaptation scaling for E neurons (scalar, typically 0-3)
...
figure;
ax = gca;
eigs_diff = eig(-eye(params.n) + params.W);
...
normalize_tau_d_for_J_eff = true;  % Set tau_d=1 for J_eff plots to compare with W

% Prepare W matrix for plotting
W_plot = full(params.W);
if omit_diagonal_in_J_eff
    W_plot = W_plot - diag(diag(W_plot));
end

% Compute color limits from W (use 90th percentile of absolute non-zero values)
if omit_diagonal_in_J_eff
...
% Use first subplot only (same layout as J_eff for consistent scaling)
...
% Get weights from E neurons (columns corresponding to E indices)
W_E = full(params.W(:, params.E_indices));
W_E_nonzero = W_E(W_E ~= 0);

% Get weights from I neurons (columns corresponding to I indices)
W_I = full(params.W(:, params.I_indices));
...
for i = 1:n_plots
    ...
    % Set colormap and color limits (same as W)
    colormap(bluewhitered_colormap(256));
    clim(global_clim);
