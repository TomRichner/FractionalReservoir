...
- **`src/presets/srnn_condition_titles.m`** — name → display text, kept **out** of the conditions on purpose. A map keyed by name is what lets a *saved* run be retitled without recomputing it; a title carried in the condition would be frozen into the run directory at compute time. This is also why `sfa3_std2` exists as a second name for physics identical to `sfa_and_std`: one regime, two labels, so the 3-condition preset can say "Multiple-Timescale Adaptation" while the others keep "SFA + STD".
...
- `presets/` — the two orthogonal knobs, together: `srnn_param_preset` (*which network*, and *which adaptation regimes*) and `analysis_run_config` (*how much compute*), plus `validate_preset_conditions`, `srnn_condition_titles` and `resolve_run_context`. See "Parameter presets vs. run modes" above. (`srnn_adaptation_conditions` was deleted 2026-09-03; presets state their own conditions.)
