...
- `nonlinearities/` — `piecewiseSigmoid`, `tanhActivation` and their derivatives (also internalized into `SRNNModel2`). The **default** activation is `piecewiseSigmoid(x, S_a, S_c)` with `S_a = 0.9`, `S_c = 0` (piecewise linear/quadratic, range [0,1], `phi(S_c) = 0.5`), bound in `set_defaults` — centering at 0 puts the resting point `x ≈ 0` in the middle of the linear region. Note `set_defaults` runs *before* constructor name-value parsing, so the bound handles capture the property-block `S_a`/`S_c`; a caller passing `'S_c'` must rebuild both handles itself. The standalone `logisticSigmoid.m` file was removed, but `logisticSigmoid(x, S_c)` = `1/(1+exp(-4*(x-c)))` / `logisticSigmoidDerivative` live as `SRNNModel2` static methods and, with `tanhActivation`, remain available as alternative activations.
...
- The `c_E` / `c_I` adaptation scaling defaults to `1/3` (a "total" adaptation budget of 1 split over three timescales). The older, much weaker `0.15/3 ≈ 0.05` convention is still hard-coded in `run_dc_lle_analysis.m`, which specifies its own bursting regime.
- The default network is E:I = 2:1 (`f = 2/3`) with `mu_I_tilde = -6*F`, the mean-balanced inhibition for that ratio; `tau_d = 1 s`.
