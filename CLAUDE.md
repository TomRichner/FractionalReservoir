...
- `nonlinearities/` — `piecewiseSigmoid`, `tanhActivation` and their derivatives (also internalized into `SRNNModel2`). The **default** activation is `piecewiseSigmoid(x, S_a, S_c)` (`S_a = 0.9`, `S_c = 0.4`), bound in `set_defaults` — matching the older `ConnectivityAdaptation/StabilityAnalysis/src/SRNNModel.m`. The standalone `logisticSigmoid.m` file was removed, but `logisticSigmoid`/`logisticSigmoidDerivative` (`1/(1+exp(-4*(x-c)))`) and `tanhActivation` remain as `SRNNModel2` static methods, selectable as alternative activations but no longer the default.
