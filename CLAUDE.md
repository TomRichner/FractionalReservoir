...
- `nonlinearities/` — `piecewiseSigmoid`, `tanhActivation` and their derivatives (also internalized into `SRNNModel2`). The **default** activation is `logisticSigmoid(x, S_c)` = `1/(1+exp(-4*(x-c)))` (centered at `S_c = 0.4`, range (0,1), unit slope at center), bound in `set_defaults` — chosen for more robust near-edge-of-chaos stability. The standalone `logisticSigmoid.m` file was removed, but `logisticSigmoid`/`logisticSigmoidDerivative` live as `SRNNModel2` static methods; `piecewiseSigmoid(x, S_a, S_c)` (`S_a = 0.9`) and `tanhActivation` remain available as alternative activations but are no longer the default.
