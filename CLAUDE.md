...
**Console verbosity is ONE setting with three levels** (2026-09-14): `verbose` = everything (per model, per job, per seed; what used to print unconditionally), `minimal` = **the default** (one line per stage or batch, one per failure, the final summary; a full `fast` pipeline transcript fits a screen per stage, which is what an agent supervising a run over the MATLAB MCP needs), `near-none` = errors and the one-line outcome of each entry point. It is `cfg.verbose` in `paper_config` and every `*_config.m`, `ctx.verbose` from `resolve_run_context`, `opts.verbose`/`cfg.verbose` on every stage, a `verbose` property on `ParamSpaceAnalysis2` and on both model classes (so it rides `model_args` into the parfor workers), and `verbose` on `make_all_paper_figures`'s prefix into every `fig_*`. Print through `vprintf(level, needed, fmt, ...)` (`src/util/`), never a bare `fprintf`; `verbose_level` ranks the names and accepts a logical for old saved objects. **Warnings and errors are never gated**, `fprintf(2, ...)` failure lines stay, and `verbose` is excluded from `resolved_defaults` and `same_config`. The `evalc` wrappers the stages used to need are gone; tests still use `evalc` to capture what they assert on. `test_verbose_levels` checks a model and a sweep print nothing at the default. **The transcript is saved with the data** (2026-09-14): `run_all_paper_analyses` writes `<run_dir>/command_window.log` and `make_all_paper_figures` `<fig_root>/command_window.log`, each under an `onCleanup`. **Since 2026-09-15 the log is written by `vlog` (`src/util/`), not `diary`**: every `vprintf` line and every `vfail` line (the ungated stderr failure line, which replaced the bare `fprintf(2, ...)` calls in the entry points and the overnight queue) is appended to whatever log is open, so the transcript is complete however MATLAB was launched. `diary` only saw the command window, and the MATLAB MCP server captures that before `diary` does, so every MCP-launched run had two empty logs (the mu5 run among them; its transcript survives only as `data/sfaEI_mu5_fast_launch_log.txt` from an `evalc` wrapper). Not captured: MATLAB's own warnings (the last one raised during each stage is appended by `run_all_paper_analyses`) and anything a parfor worker prints. Bare `fprintf` in library code bypasses the log too, which is one more reason to use `vprintf`.
...
- **`C = 1` works, and is for figures only.** A one-cell-type network builds, runs,
  ...
  scripts to select axes conditionally. A `C = 1` preset writes `1 x 1` `tau_a` rows
  in its own conditions; `test_pairs_single_celltype` checks both shapes.
...
- **`src/presets/srnn_param_preset.m`** — *which network*. `[d, model_class] = srnn_param_preset(name)` returns a struct of model overrides **plus the model class they are written for** (the name is only a lookup key; the struct is what reaches the model). Assign it to `psa.model_defaults` and layer tweaks on with plain field assignment. Presets carry physics **including** swept axes like `n`/`f`/`level_of_chaos` — a grid axis overrides the preset for the sweep that varies it, while sweeps holding that axis fixed use the preset's value. They must **not** carry `n_levels`/`n_reps` (not model properties), the run_mode timing knobs, or condition fields. The second output exists because the two model classes have **disjoint** parameter vocabularies, so a preset that did not carry its class would only fail later inside `validate_model_defaults`; it defaults to `'SRNNModel2'`. The third output is the **conditions**, which is how a preset states its own synapse routes.
  ...
  **EACH PRESET STATES ITS OWN ADAPTATION REGIMES, in full, in its own `case` block** — names, `tau_a` rows and `synapse_config` written out as a literal cell array. Read the block and you know the experiment; there is no second file to consult. **One exception (TR, 2026-09-14): within a run/config/preset bundle, presets MAY be related.** A derived preset that changes one named thing about its counterpart (`..._noise0_dualStd_3cond_mu8p25` calls `srnn_param_preset` on `..._noise0p025_...` and sets `sigma_u_noise = 0`, for memory capacity) is clearer than a 30-line copy that could drift. A preset must NEVER chain to one that belongs to another config's bundle; the case block says which config it belongs to.

  This replaced `srnn_adaptation_conditions`, deleted 2026-09-03, which turned a regime-set *name* (`'standard'` / `'timescales'` / `'single_multi'`) into condition structs. Two reasons it went: a preset stated its network in full but **delegated its experiment**, so understanding one meant tracing another file's locals; and only 2 of 10 presets named a set explicitly, so the other **8 inherited `'standard'` by default** and editing that one case silently changed eight networks' experiments. Regime *names* still mean one thing everywhere — `no_adaptation` is identical across the 3-, 4- and 7-condition presets, and `test_preset_conditions` asserts it — but that is now a checked invariant rather than a structural one.

  A condition is a plain struct of model-property overrides plus a `name`; `run_single_job` appends every field except `name` as a constructor argument. Note `struct('tau_a', {row})` needs the **double brace** — `struct()` given a bare cell builds one struct *per cell entry*, so the single-brace form silently yields a struct **array** instead of a condition.

  `SRNNModel2` presets spell adaptation as `n_a_E`/`n_b_E` **counts** (the class auto-fills `tau_a_E`); `SRNNCellTypePairs` presets spell it as an explicit `tau_a` cell plus a `synapse_config` — `tau_a`, not `n_a`, because on Pairs `n_a` is Dependent on `tau_a` and read-only, the reverse of `SRNNModel2`.

  **CONDITION NAMES STATE THEIR ADAPTATION STRUCTURE**: `no_adaptation`, or `sfaX_stdY` where X and Y are the number of SFA and depression timescales actually in use, plus `_stfZ` where facilitation is on. So a name means exactly one thing in every preset. Before 2026-09-03 the names were mechanism-based, and **`sfa_and_std` meant four different things** — 3 SFA + 2 STD in the paper's networks, 3 + 1 in `bursting_pairs` and both `SRNNModel2` presets, 3 + 0 in `sompolinsky_pairs`, 1 + 1 + STF in `single_neuron_stf`. Two run directories could carry the same condition folder name and different physics; `7cond` already had a *separate* regime genuinely called `sfa3_std1`.

  Consequences worth knowing: `sompolinsky_pairs` has **two** conditions, not four — it has no routes, so its old `std_only` was physically identical to `no_adaptation`, which the mechanism names concealed. `single_neuron_stf` is the only preset using `_stf`, and it needs it: without the suffix its full regime would be `sfa1_std1`, which is already the 3-condition preset's SFA+STD *without* facilitation.

  **This is a convention, not a rule.** Conditions stay fully general — one comparing `level_of_chaos` would not fit the scheme at all — so it is checked in `test_preset_conditions` as *names must not lie* (a name matching the pattern must match its actual counts; non-matching names are skipped), **not** enforced in `validate_preset_conditions`, which stays free of domain knowledge.
- **`src/presets/full_adaptation_condition.m`** — given a conditions cell, returns the name of the most-adapted regime. Needed because **four different names** now mean "fully adapted" (`sfa3_std2`, `sfa3_std1`, `sfa1_std1_stf1`, `sfa3_std0`), so the figures and tables that want one representative network cannot hardcode a literal. They used to hardcode `'sfa_and_std'`, and four figures plus the tau sweep failed with `NoSuchCondition` the first time a preset named that regime differently. `build_from_preset(preset, '')` resolves it; pass a name to override. **Beware MATLAB's `regexp`**: it returns no token for a trailing optional group even when that group matches, so `'sfa1_std1_stf1'` against `sfa(\d+)_std(\d+)(?:_stf(\d+))?` silently yields two tokens and reads STF as zero. Named tokens behave the same. Both this helper and the test therefore use **two exact patterns** instead.
- **`src/presets/validate_preset_conditions.m`** — called at the end of `srnn_param_preset`, so a malformed set fails **in the file you are editing** rather than inside a parfor worker mid-sweep. Conditions were previously the only unvalidated layer of a run: `model_defaults` and grid axes are checked up front, but condition fields reached only the model constructor. Five checks, deliberately **general** — it knows nothing about adaptation: scalar structs (catching the brace trap above); `name` present, char and unique; **every condition sets the same field set** (an omitted field does not fall back to a sibling — it falls back to `model_defaults`, silently running a different experiment); every field publicly settable on the model class; and every name a key of `srnn_condition_titles`, so no figure falls back to a raw snake_case panel title. It uses `meta.class` directly — `ParamSpaceAnalysis2.srnn_property_info` is a **private** static, and this also keeps `src/presets/` from depending on `src/analysis/`.
- **`src/presets/srnn_condition_titles.m`** — name → display text, kept **out** of the conditions on purpose. A map keyed by name is what lets a *saved* run be retitled without recomputing it; a title carried in the condition would be frozen into the run directory at compute time. Titles are **mostly structural** (`sfa3_std1` → "SFA 3τ + STD 1τ"), with two deliberate exceptions: `sfa1_std1` and `sfa3_std2` — the 3-condition preset's adapting regimes, i.e. the paper's actual comparison — are titled "Single-Timescale Adaptation" and "Multiple-Timescale Adaptation", because that is the claim being made rather than how it is implemented. That those words could be restored after the rename, in one file, without touching a preset or a saved run, is the point of keying titles by **name** instead of carrying one inside each condition. It also keeps the **pre-rename names as legacy keys**, so figures regenerated from older run directories still get sensible panel titles. Two other maps key on condition names and must gain any new one: `manuscript_style` (colours *and* short labels) and `load_and_plot_lle_by_stim_period`, which now delegates its titles here rather than carrying a private copy.
- **`src/presets/analysis_run_config.m`** — *how much compute*. `analysis_run_config(analysis, run_mode, preset_defaults)` returns `n_levels`, `n_reps`, and a `cfg.model` struct of timing settings (`ode_solver`, `fs`, `T_range`, and `lya_T_interval` only when it should be set).
  ...
  **`src/presets/run_mode_names.m` is the ONE list of run modes** — `fast`, `medium`, `medium2`, `production` — and every stage validates against it. There used to be four separate lists: `analysis_run_config` accepted five modes while `run_memory_capacity`, `run_memory_capacity_example` and `run_eig_heatmap` each had a private switch accepting three. A mode valid for the sweeps was therefore invalid downstream, and since `run_all_paper_analyses` wraps each stage, that showed up as stages producing nothing *after* the sweeps had already run. On 2026-09-03 `single_multi_TS` did exactly that at `'fast2'`: three stages died, two figures failed on missing data, and `fig_memory_capacity` fell back to a twelve-day-old `.mat` and reported **success**. `'fast2'` was removed (it was `'fast'` with more reps and a longer window on two of three sweeps); `medium2` now collapses to `medium` in the stages that have no sweep dimension for it to affect. `run_all_paper_analyses` **pre-flights the mode before any compute**, and `test_run_modes` asserts every stage names the canonical list.

  **The two knobs are no longer fully orthogonal, and this is where they meet.** Every cell names *two* integrators — deterministic (`'rk4'` for fast/medium/medium2, `'ode45'` for production) and stochastic (`'sra1'` everywhere) — and the **preset** picks between them: `sigma_u_noise > 0` selects the stochastic one. That is why the third argument exists, and why the three sub-scripts resolve `preset_defaults` *before* calling this. Selecting here is what leaves everything else intact: `merge_struct` precedence is unchanged (`cfg.model` still wins), `ode_solver` stays banned from presets, and a σ = 0 preset is bit-identical to before the mechanism existed. `cfg.sde_solver` / `cfg.is_stochastic` live at the top level of `cfg`, never inside `cfg.model` — only `cfg.model` is merged into `model_defaults`, and neither is a model property. Note a stochastic `production` run is fixed-step and so does **not** carry `ode45`'s adaptive tolerance: there is no adaptive SDE solver, because step-size control is meaningless once increments are tied to the step.
...
- `model/` — the three classes (`SRNNModel2`, `SRNNCellTypePairs`, `SRNN_ESN_reservoir`) and everything that defines what a network *is*:
  ...
  - `integrators/` — `ode_rk4.m`, `sde_fixed_step.m`, and **the solver registry both model classes share**: `solver_names`, `deterministic_solver_names`, `stochastic_solver_names`, `resolve_solver`, `check_ode_solver`, `check_noise_settings`. These were statics duplicated verbatim on both classes (identical bodies, only the error identifier differing), and `check_noise_settings` existed on `SRNNModel2` alone — which `SRNNCellTypePairs.validate()` called, so **every Pairs model in the repo depended on `SRNNModel2` existing**. Each raising function takes an `err_id_prefix` so a class still raises its own identifier; note `SRNNModel2` passes `'SRNNModel'`, not its own name, a pre-existing inconsistency `test_ode_solver_name` asserts. That test now checks neither class *re*defines the registry, so a copy cannot creep back.
  ...
  - `connectivity/` — **`RMTBlocks.m`** is the generator all three model classes now use. It extends Harris (2023) to full D×D block statistics indexed **(postsynaptic, presynaptic)**, so `mu_EE` can differ from `mu_IE`, and it supplies the bulk radius `R` and the outlier eigenvalues `lambda_O`. `RMTMatrix.m` remains as a **frozen reference implementation**: no source calls it, but `test_RMTBlocks_equivalence` asserts `RMTBlocks` reproduces it bit-for-bit in the `SRNNModel2` configuration, and that test *skips silently* if the class is absent — so deleting it would downgrade a test rather than fail one. Note `RMTBlocks` returns a **dense** matrix; the CellTypes classes re-sparsify after assembly.

  `RMTCellTypes.m` was **deleted 2026-09-02**. CLAUDE.md previously said it was "no longer used by the model classes", which was wrong: `SRNNCellTypePairs` called its `allocate_counts` and `make_type_indices` statics, and `allocate_counts` partitioned `(n, f)` by **largest remainder** where `RMTBlocks` uses **cumulative rounding**. Two algorithms for one partition, and the model indexed its entire state vector by one while `W` was built from the other. They agree for every `C = 2` model — which is why no preset was ever affected — and disagree for ~22% of `C >= 3` draws, where the model applied type-*q* parameters to a neuron whose outgoing synapses `W` had built as type-*q'*: a silent Dale's-law violation. `get.n_per_type` now uses cumulative rounding inline, so there is one algorithm. **If you touch it, it must stay the same function of `(n, f)` that `RMTBlocks.get.n_per_type` is** — `scripts/tests/test_celltype_partition.m` enforces that by reading the partition back out of `W`'s column signs at sizes chosen to hit the divergence.
  - **The nonlinearities are statics on the model classes**, not a folder. `src/model/nonlinearities/` was **deleted 2026-09-02**: it held `piecewiseSigmoid`, `tanhActivation` and their derivatives with **zero callers**, and had already **drifted** — the standalone copies lacked the `~isscalar(c)` branch both classes carry, so they would have silently mishandled a per-neuron `S_c` rather than erroring. The nonlinearity belongs to the class surface anyway: it is selected by name, parameterised by `S_a`/`S_c` properties, and that per-neuron branch makes it a method over model state.

    Select one with `activation` (see "Conventions" below), not by building a handle. The **default** is `'logistic'` = `logisticSigmoid(x, S_c)` = `1/(1+exp(-4*(x-c)))` (centred at `S_c = 0.4`, range (0,1), unit slope at centre) — chosen for more robust near-edge-of-chaos stability; `'piecewise'` (`S_a = 0.9`) and `'tanh'` are the alternatives. Each model class carries its **own copy** of these statics, so a fix to one is not a fix to the other. Reach for `SRNNCellTypePairs`'s when a figure needs the curve directly (`fig_FI_curve` does).
- `presets/` — the two orthogonal knobs, together: `srnn_param_preset` (*which network*, and *which adaptation regimes*) and `analysis_run_config` (*how much compute*), plus `validate_preset_conditions`, `full_adaptation_condition`, `srnn_condition_titles`, `run_mode_names` and `resolve_run_context`. See "Parameter presets vs. run modes" above. (`srnn_adaptation_conditions` was deleted 2026-09-03; presets state their own conditions.)
- `analysis/` — `ParamSpaceAnalysis2` plus everything that *runs* a sweep: the orchestrator `run_all_analyses` and its three sub-analyses (all functions taking `ctx`), `run_memory_capacity`, `run_eig_heatmap`, `run_memory_capacity_example`, and the standalone tools `run_dc_lle_analysis`, `check_sensitivity_sim`, `run_overnight_queue`. `mu_block_from_preset` is here too — it decides the mu sweep ranges and is shared by the 1-D and grid sweeps so they cannot drift apart.

  `run_eig_heatmap` and `run_memory_capacity_example` used to sit inside the figure folders whose data they produce, and defaulted to writing their `.mat` next to their own `.m` — which is exactly how two figures came to read four-day-old data. They are analyses; they belong here.
- `figures/` — the `fig_*.m` functions, **flat**, plus `plot_memory_capacity`, `replot_memory_capacity`, `plot_memory_capacity_combined`. They were one folder per figure under `scripts/presentations/Stability_Manuscript/`, which earned its keep while each folder also held that figure's outputs; once those moved to `figs/`, they were 17 directories holding one file each, with `fig_energy_landscape.m` four levels deep inside another figure's tree.
...
- `plotting/` — colormaps, line/scatter helpers, time-series panel plots, `param_space_plots/` for post-hoc visualization, `plot_saving/save_some_figs_to_folder_2.m` (used by everything that writes figures). **Kept separate from `figures/` deliberately**: these are primitives `SRNNModel2.plot()` itself calls, so they belong to the model's surface, whereas `figures/` is manuscript composition on top. Merging them would make the model depend on the manuscript.
- `util/` — cross-cutting helpers with no layer of their own: `merge_struct`, `struct2namevalue`, `restart_parpool`, `capture_git_provenance`, `verify_shared_build`, `write_run_parameters_md`.

### `scripts/` — the three things a human invokes

Everything else became library code and moved into `src/` on 2026-09-02. Of the
~55 `.m` files that used to live under `scripts/`, exactly one
(`reproduce_paper_run.m`) was a script you actually run; the rest were functions
called by something else, so the `scripts` / `src` line had stopped meaning
anything.
...
### Figure conventions
  ...
  **Figure polish and the mu = 5 bundle (TR, 2026-09-14 evening).** `sfaEI_mu5_fast_config` / `_run` was the bundle of that day (superseded 2026-09-15 by `sfaEI_mu7_fast_config` / `_run`: `mu_tilde_relative` 7, the four mu sweeps AND `level_of_chaos` ("All Weights") spanning -75 % to +75 % of the preset in the 1-D sweeps and the joint grid, fast sensitivity 5 levels x 5 reps, route labels pre -> post so mu_EI reads I->E, Okabe-Ito blue for the multi-timescale regime and a 20 %-darker orange for the single-timescale one, and statistics moved off the panels into each figure's `_table.md`; the mu5 files stay for reproducibility): three stand-alone-plus-chained presets ending in `_mu5` (`mu_tilde_relative` 5 on every route, `tau_a_spread` 0.25, no external input; a `noise0` twin for memory capacity and a `steps5s` twin for `run_local_lyapunov`). The noStim mu8p25 bundle it replaced was deleted (its presets chained across bundles). Global changes made with it: the four mu sweep axes span **-50 % to +50 %** of the preset (`mu_block_from_preset` factors `[0.5 1.5]`; earlier runs used 0.25x-3x and cannot be pooled); the joint `param_space` sample is **at least 128 points in every mode**; `plot_sensitivity` and `fig_sfa_EOC_allStd` **drop** values outside their histogram range instead of folding them into overflow rows, and `fig_sensitivity_medians` clips at the registry `median_ylim`; `fig_example_timeseries` shows 25 E + 25 I neurons with the per-neuron `(c/K) sum a`; `fig_adaptation_methods` runs `single_neuron_dualStd` noise-free from t = -15 s; the eig heatmaps are grayscale on white; `fig_introductory_concepts` is one figure (`Fig_Intro_Concepts`); `fig_local_vs_finite_lle` keeps only its top row; `fig_transient_amplification` saves two figures; `run_memory_capacity_example` simulates a 75-s test window at fast so the 60-s reconstruction is full; `fig_energy_landscape` and `fig_EI_param_space` are no longer registered (functions kept).
...
**Memory capacity** was `scripts/memory_capacity/` and is now split by layer like everything else: `run_memory_capacity(cfg)` (the paired-trial ensemble, a function with its own `mc_run_config` local cost table) → `src/analysis/`; `plot_memory_capacity` and `replot_memory_capacity` → `src/figures/`; `example_memory_capacity.m` → `scripts/examples/`; `test_sample_hold_mc.m` → `scripts/tests/`.

`SRNN_ESN_reservoir` subclasses **`SRNNCellTypePairs`** (re-parented 2026-09-02, commit `7966fc7`), so the paper pipeline is now **single-class** — `mc_esn` was the last `SRNNModel2` preset in it and is deleted; `mc_pairs_dualStd` replaces it. Three consequences worth knowing:

- **The `'synaptic'` readout takes one route, and checks first.** Depression is per *route* on Pairs, so `theta_j^{q→s} = r_j ∏b ∏g` differs by target and "the synaptic output of neuron *j*" need not exist. `SRNN_ESN_reservoir.assert_route_redundancy` requires, **per presynaptic type**, that all of that type's outgoing routes carry identical STD *and* STF, then reads route 1. That is exact, not an approximation: identical routes integrate the same ODE from `b = 1` driven by the same `r_j`, so the trajectories are bit-identical. Per *presynaptic type* rather than globally, because neuron *j* of type *q* only has routes `q→*`; "E→\* depressed, I→\* not" is well defined and is accepted. `readout_signal = 'rate'` is the escape hatch for a preset with genuinely per-route synapses. Enforced by `scripts/tests/test_esn_route_redundancy.m`, which also asserts the bit-identity numerically.
- **`mc_run_config` selects its integrator** the way `analysis_run_config` does — `det_solver` at `sigma_u_noise = 0`, `sde_solver` above it — plus an explicit `'ode_solver'` override on both entry points. The override exists because `'sra1'` is legal at σ = 0 (`sde_fixed_step` treats an absent noise tensor as zero) and naming it is the only way to make an MC run use the same integrator as the sweeps with noise off. It previously hardcoded `'rk4'`, which meant a noisy preset could not run MC at all.
- **MC runs save snake_case condition keys**, like every other run directory, with display text from `srnn_condition_titles` via `src/figures/helpers/mc_display_names.m`. Unknown keys pass through unchanged so pre-2026-09-02 runs still plot.
...
- **`lya_method = 'topk'` is the K largest exponents by the discrete QR method** (`lya_K`, default 10; 0 = N), in the shared core `src/model/lyapunov/lyapunov_topk.m` that both classes call with their own Jacobian. It propagates an N x K basis by fixed-step Heun along the **stored** fiducial states, re-orthonormalises every `lya_dt` with an economy QR (diag(R) forced positive), and honours the window and warmup like the other two. On `SRNNCellTypePairs` the product J·Y is **matrix-free** (`jacobian_times`, passed as `opts.jac_times`): assembling the sparse Jacobian cost ~16 ms per step at N = 4000 against 0.3-3 ms for the product, so the class applies the blocks directly. **`jacobian_times` must equal `compute_Jacobian_fast * Y` to round-off; any change to `compute_Jacobian_fast` or `dynamics_fast` must be mirrored there and `test_jacobian_times` rerun.** `SRNNModel2` passes no `jac_times` and uses the assembled fallback. It also returns `h_KS`, `n_positive`, `D_KY` (+ `D_KY_resolved`), the conditioning `cond_max` and `Q_final`. `'qr'` stays the ode45 full-spectrum reference; `'topk'` at K = N is ~35x faster and verified against it and against Liouville (sum of exponents = mean trace J) in `test_lyapunov_topk`. Spectra **nest** across K on the unsorted columns. Do not compare individual exponents between methods on a **stable** net: its slow band is nearly degenerate and finite-time exponents there depend on the initial basis (`docs/notes/Lyapunov_estimation_methods.md`).
- **The sweeps use `'topk'`, and every job stores the spectrum-derived measures** (TR, 2026-09-12). `analysis_run_config` sets `lya_method = 'topk'`, `lya_K = 15`, `lya_K_auto = true`, `lya_K_max = 60`, `lya_dt = 0.05` for every analysis and mode; a config wanting the Benettin comparison sets `cfg.model.lya_method = 'benettin'` after calling it (same seeds, same networks, only the estimator differs; `same_config` refuses to pool the two). `lya_K_auto` doubles K on the stored trajectory while the Kaplan-Yorke dimension is unresolved, up to `lya_K_max`; the retry nests exactly. **`SRNNCellTypePairs.lya_summary()` is the one definition of the per-run scalars** (λ1, gap, n₊, h_KS in bit/s, D_KY + resolved flag, K used, conditioning, λ1 drift, the transient-divergence scalars from the leading direction's local rates, the leading vector's x/SFA/STD/STF fractions) and `ParamSpaceAnalysis2.run_single_job` copies all of them into every result, NaN where a method cannot supply one (`lya_summary_fields` is the list). **`src/presets/sweep_metrics.m` is the one registry of measures the figures plot**: label, ranges, zero line, figure-tag stem, and `in_sheets` (lle, r, hks, dky get their own sensitivity / medians / param-space / E:I sheets, tags `Fig_..._<stem>`). Add a measure there, not in a figure. `write_run_parameters_md` prints a Lyapunov-quality table per sweep and condition (share with D_KY resolved, share with n₊ at K, K used, worst conditioning, median drift). Two further stages: `run_eig_heatmap` records the **numerical abscissa** max eig((J+Jᵀ)/2) beside the spectral abscissa at every sampled state (`fig_transient_amplification`), and `run_lyapunov_spectrum` + `fig_lyapunov_spectrum` give the spectrum's shape per regime, noise on and off, at K = 50/200/300 by mode.
- **The transient-gain stage measures amplification with adaptation FROZEN vs ACTIVE** (TR, 2026-09-13). `run_transient_gain` + `fig_transient_gain` + `fig_transient_gain_excursions`, registered after `lyapunov_spectrum`. The measure is the x-in / x-out gain G(t) = ‖P_x Φ(t) P_xᵀ‖₂ of the tangent flow (perturb only dendritic states, measure only dendritic states: invariant to how a, b, g are scaled, no stability assumption), from `SRNNCellTypePairs.transient_gain`, which propagates the n dendritic unit vectors with `lyapunov_topk`'s Heun step and **no re-orthonormalisation**. Three propagators per sampled state: `frozen_x` (dense J_xx fixed at the state, the conventional rate-network picture), `frozen_full` (the full J fixed, via `jacobian_times` with a constant state) and `active` (J(S(t)) along the stored trajectory); the first gap is adaptation's dynamic feedback, the second nonstationarity. From the same n × n block it also reads the noise-average gain ‖·‖_F/√n, the gain along the E/I difference and sum modes and along the leading Lyapunov direction (`leading_direction_at`, a K = 1 propagation to the sample), and the peak's optimal input direction. Two sampling rules: regular states, and excursion onsets / quiet midpoints of the leading local rate (`excursion_samples`), for the intermittency question. Do not expect the frozen propagators to bound the active one: on small nets the active gain came out **larger** (state drift through regions of stronger local expansion); see the 2026-09-13 commit messages and `docs/notes/Non_normal_amplification.md` §6.
- **The tau sweep moves both cell types** (2026-09-14). `tau_a_EI` is a Dependent alias on `SRNNCellTypePairs` whose setter writes one ladder into every `tau_a{q}` and whose getter asserts the ladders are equal (`TauAMismatch`); `tau_a_E` still moves type 1 only. `run_tau_sensitivity_analysis` picks the axis from the condition -- `tau_a_EI` when >= 2 types carry identical non-empty ladders, else `tau_a_E` -- and writes `tau_levels.md` / `tau_levels.mat` (the resolved E and I ladders per level) as the manifest that both types were swept. `fig_sfa_EOC_allStd` reads whichever axis the run used. `test_tau_a_EI_alias`.
- **The numerics stage is a gate, not a report** (2026-09-14). Five checks per condition: A noise-free reshoot order, B noisy strong order, L Benettin ode45 vs SRA1, C Benettin vs QR vs top-K on the reduced network, J analytic Jacobian vs central finite differences (`SRNNCellTypePairs.finite_difference_jacobian`, kink rows excluded). The acceptance thresholds were fixed in code on 2026-09-14 before any run and are saved in `settings.acceptance`; the `verdict` (value / threshold / pass per check) is saved in the .mat and written to `numerics_verdict.md` with a PASS/FAIL line per manuscript claim. Fast windows fail L and C by design; judge on medium or production.
- **`run_eig_heatmap` samples imbalance examples** (2026-09-14). `cfg.examples` (default `mu_EE_relative` x 0.5 / 1 / 1.5, all conditions on the same seeds) stores per example and condition the pooled eigenvalues, top-K lambda_1, **mean rate** and the realised E:I weight balance B_E; the legacy top-level variables are the reference example, so `fig_eig_heatmap` and `fig_transient_amplification` read as before. `fig_eig_heatmap_imbalance` is the rows-by-columns sheet. `test_eig_heatmap_stage`.
...
- **The SFA ladder can be per-neuron too** (`SRNNCellTypePairs` only). `tau_a_spread` (`1 x C`, dimensionless, default zeros) is a log-normal spread applied to the two **ends** of each type's `tau_a` ladder, with the interior rungs following in log space at the nominal's own position — for a `log_ladder` nominal that is "jitter the endpoints and re-run logspace". `build()` draws it (seed `tau_a_seed`, default derived from `rng_seeds(1)`, twister-pinned, RNG state restored so `W`, setpoints, stimulus and `x0` are untouched) into the read-only `tau_a_matrix`, a `1 x C` cell of `n_q x n_a(q)` matrices (one **row per neuron**; empty when no spread). `get_params` always emits `params.tau_a_matrix{q}` at that size, so `dynamics_fast`, `compute_Jacobian_fast` and `jacobian_times` have one code path and a zero spread is bit-identical to before (`test_SRNNCellTypePairs_tau_a_spread` pins that against a saved reference). Ordering is kept by a **minimum log-separation floor** (2026-09-14): a neuron whose two end draws fall closer than a quarter of the nominal log-span is pushed apart symmetrically about its log-midpoint, the count is recorded in `tau_a_clipped`, and any spread builds (the old `validate` error refused spread 0.25 on the tau sweep's short [0.25 1] s ladder); so `tau_a_E(end)` still means "the slowest" for every neuron. Why it exists and what it does to the Lyapunov spectrum: the property header and `docs/notes/Lyapunov_estimation_methods.md`. `c_eff = c / K` is unaffected.
