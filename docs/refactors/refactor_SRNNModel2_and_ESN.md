...
## 7. Verification Helper: `verify_shared_build`

Static method on `SRNN_ESN_reservoir` (internalized from `src/verify_shared_build.m` in commit `49945cb`).
...
3. **In `build_stimulus()` override**, you must set:
   ...
   - `obj.S0` — initial state vector (use `obj.initialize_state(obj.get_params())`)
...
7. **Use `SRNN_ESN_reservoir.verify_shared_build()`** in scripts comparing multiple conditions
8. **Call activation/Jacobian functions via the class** — use `SRNNModel2.piecewiseSigmoid(...)`, `SRNNModel2.compute_Jacobian_fast(...)`, etc. instead of standalone function files

---

## 9. Function Internalization (commit `6e2c58f`, `49945cb`)

All standalone helper functions were internalized as static methods to make the classes standalone. See [internalize_functions_into_classes.md](file:///Users/tom/Desktop/local_code/FractionalReservoir/docs/refactors/internalize_functions_into_classes.md) for the complete listing.

### Summary of internalized categories

| Category             | Count        | Target class       | Access                           |
| -------------------- | ------------ | ------------------ | -------------------------------- |
| Activation functions | 6            | SRNNModel2         | Static (public)                  |
| Stimulus generation  | 1            | SRNNModel2         | Static protected                 |
| Jacobian computation | 2 + 1 helper | SRNNModel2         | Static (public) / Static private |
| Plotting functions   | 11           | SRNNModel2         | Static protected                 |
| Verification         | 1            | SRNN_ESN_reservoir | Static (public)                  |

### What was NOT internalized

- `RMTMatrix.m` — separate class, shared dependency
- `setup_paths.m` — still needed for `RMTMatrix` and other shared utilities
