classdef ParamSpaceAnalysis2 < handle
    ...
    methods (Access = private)
        function set_default_conditions(obj)
            ...
            %
            % SRNNModel2-shaped, and a fallback only: every preset supplies its
            % own conditions, so a pipeline run never reaches these. Names follow
            % the sfaX_stdY convention -- X SFA timescales, Y depression
            % timescales -- so that a run built on these defaults is keyed the
            % same way as one built from a preset.
            obj.conditions = { ...
                struct('name', 'no_adaptation', 'n_a_E', 0, 'n_b_E', 0), ...
