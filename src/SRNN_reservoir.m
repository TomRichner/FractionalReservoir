...
%
% Optional 6th argument noise_ex (n x T matrix): pre-generated process noise
% for each neuron at each time point. Set to [] or omit for no noise.
%
% State organization: S = [a_E(:); a_I(:); b_E(:); b_I(:); x(:)]

persistent u_interpolant noise_interpolant t_ex_last;
...
if isempty(u_interpolant) || isempty(t_ex_last) || ...
    ...
    % Also set up noise interpolant if provided
    if nargin >= 6 && ~isempty(noise_ex)
        noise_interpolant = griddedInterpolant(t_ex, noise_ex', 'linear', 'none');
    else
        noise_interpolant = [];
    end
    t_ex_last = t_ex;
...
%% interpolate noise vector (if provided)
if ~isempty(noise_interpolant)
    noise = noise_interpolant(t)';
else
    noise = zeros(params.n, 1);
end
...
% dx/dt = -x/tau_d + W*(b.*r) + u + noise
% Corrected: Apply b (presynaptic depression) here
dx_dt = (-x + W * (b .* r) + u + noise) / tau_d;
