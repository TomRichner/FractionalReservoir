function [t_out, S_out, params, lya_results] = full_SRNN_AM_pulse(u_ex_scale, n_a_E, n_b_E, level_of_chaos, save_dir, save_figs, save_workspace, note)
% FULL_SRNN_AM_PULSE Runs the SRNN simulation with AM pulse train paradigm.
%
% Inputs:
...
%   note          - Optional string for naming figures (default: 'SRNN_AM')
...
params.tau_a_E = logspace(log10(0.2), log10(20), params.n_a_E);  % Logarithmically spaced from 0.1 to 10
params.tau_a_I = logspace(log10(0.2), log10(20), params.n_a_I);  % Logarithmically spaced from 0.1 to 10
...
params.tau_d = 0.05;  % dentdritic time constant, seconds

%% ICs
S0 = initialize_state(params);

%% External input
% Configure AM pulse input
pulse_config.amp_max = 3;
pulse_config.width_ms = 0.5;
pulse_config.neuron_frac = 0.5; 
pulse_config.pulse_period = 0.2;
pulse_config.mod_freq = 0.5;
pulse_config.mod_depth = 0.5;

% Generate external input
[u_ex, t_ex] = generate_AM_pulse_train(params, T, fs, params.rng_seeds(2), pulse_config);
...
plot_deci = 1; % No decimation to ensure we see the pulses
...
% Use the paired pulse plotter as it handles high frequency data well, or standard one
% Let's use standard one but it might be crowded. 
% Actually, let's use plot_SRNN_tseries but maybe we need a custom one for AM?
...
if save_figs
    if ~isempty(save_dir)
        ...
        save_some_figs_to_folder_2(figs_folder, note, fig_vec, {'fig', 'png'});
    else
