...
switch name
    ...
    case 'celltype_pairs_Sc0p2_noise0p025_dualStd_3cond_mu8p25'
        % The 3-condition paper network with the mean connectivity 50% STRONGER
        % on all four routes. Copied from ..._dualStd_3cond (TR, 2026-09-04).
        % Changed:  mu_tilde_relative  5.5 -> 8.25  (x1.5, every block)
        % Everything else -- n, sigma, nonlinearity, noise, c, depression
        % timescales, regimes -- is identical, so a run here is directly
        % comparable with a 3cond run and differs in exactly one number.
        %
        % mu scales the E and I MEAN weights together, so E/I balance is kept
        % while the outlier eigenvalues (which sit at the block means) move
        % outward. sigma is untouched, so the bulk radius R does not change.
        model_class = 'SRNNCellTypePairs';
        d = struct( ...
            'n',                    500, ...
            'indegree',             100, ...
        ...
        % Regimes as in ..._3cond: none / one timescale each / many each.
        taus     = log_ladder(0.25, 10, 3);
        sfa_off  = {zeros(1,0), zeros(1,0)};
        sfa_one  = {taus(1),    zeros(1,0)};
    ...
    case 'celltype_pairs_Sc0p2_noise0p025_tripleStd_3cond_mu8p25'
        ...
        std_all = struct();
        std_all.E.E.std = triple_std;   std_all.E.I.std = triple_std;
        std_all.I.E.std = triple_std;   std_all.I.I.std = triple_std;
        std_one = struct();
        std_one.E.E.std = single_std;   std_one.E.I.std = single_std;
        std_one.I.E.std = single_std;   std_one.I.I.std = single_std;
        std_off = struct();
        ...
        conditions = { ...
            struct('name','no_adaptation', 'tau_a',{sfa_off}, 'synapse_config',std_off), ...
            struct('name','sfa1_std1',     'tau_a',{sfa_one}, 'synapse_config',std_one), ...
            struct('name','sfa3_std3',     'tau_a',{sfa_all}, 'synapse_config',std_all) };

    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_noise0p025_dualStd_3cond_mu8p25'
        % As ..._dualStd_3cond_mu8p25 with two changes (TR, 2026-09-09):
        %   SFA ON BOTH CELL TYPES.  c  [0.5 0] -> [0.5 0.5], and the tau_a rows
        %       in every adapting regime carry the same ladder on I as on E.
        %       Every other paper preset adapts E only.
        %   PER-NEURON SETPOINT.  sigma_S_c  [] -> [0.1 0.1]: each neuron draws
        %       S_c_i = 0.2 + 0.1*randn (mu_S_c stays empty, so the centre is the
        %       shared S_c). The draw is seeded from rng_seeds(1), so a reps sweep
        %       varies the setpoints with the network, and it restores the RNG
        %       so W, the stimulus and x0 are unaffected.
        % Everything else -- n, mu 8.25, sigma, noise, depression ladder -- is
        % identical to dualStd_3cond_mu8p25.
        %
        % NOT IN THE ONE-STRUCT FAMILY. Its sfa1_std1 and sfa3_std2 carry SFA on
        % I, so they are deliberately NOT the same structs as the other paper
        % presets' regimes of those names -- the timescale COUNTS match, which is
        % what the names promise, but the physics differs by design.
        % test_preset_conditions lists the one-struct family explicitly and this
        % preset is not in it.
        model_class = 'SRNNCellTypePairs';
        d = struct( ...
            'n',                    500, ...
    ...
    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu5'
        % THE sfaEI_mu5_fast BUNDLE'S NETWORK (TR, 2026-09-14). A stand-alone
        ...
        % three changes:
        %   mu_tilde_relative  8.25 -> 5 on every route (sign by presynaptic
        %       type). The mu8p25 family is "1.5 x 5.5"; TR chose a baseline of 5.
        ...
        d = struct( ...
            ...
            'cell_type_names',      {{'E', 'I'}}, ...
            'f',                    [0.5 0.5], ...
            'mu_tilde_relative',    [5 -5; 5 -5], ...     % (post <- pre), multiples of F
            'sigma_tilde_relative', [1.5 1.5; 1.5 1.5], ...     % multiples of F
            'level_of_chaos',       1.0, ...
            'activation',           'piecewise', ...
            'S_a',                  0.8, ...
    ...
    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0_dualStd_3cond_mu5'
        % Memory-capacity twin of the mu5 noStim preset: Wiener process off
        % (the config names sra1 as the MC integrator). Same bundle
        % (sfaEI_mu5_fast_config), chained under the within-bundle rule.
        [d, model_class, conditions] = srnn_param_preset( ...
            'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu5');
        d.sigma_u_noise = 0;

    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_steps5s_noise0p025_dualStd_3cond_mu5'
        % Stimulus-staircase twin of the mu5 noStim preset for run_local_lyapunov:
    ...
    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7'
        % THE sfaEI_mu7_fast BUNDLE'S NETWORK (TR, 2026-09-15). A stand-alone
        % copy of ..._sfaEI_Sc0p2sig0p1_noise0p025_dualStd_3cond_mu8p25 (SFA on
        % both cell types, per-neuron setpoint S_c_i = 0.2 + 0.1 randn) with
        % three changes:
    ...
    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0_dualStd_3cond_mu7'
        % Memory-capacity twin of the mu7 noStim preset: Wiener process off
        % (the config names sra1 as the MC integrator). Same bundle
        % (sfaEI_mu7_fast_config), chained under the within-bundle rule.
        [d, model_class, conditions] = srnn_param_preset( ...
            'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7');
        d.sigma_u_noise = 0;

    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_steps5s_noise0p025_dualStd_3cond_mu7'
    ...
    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7revised'
        % THE mu7revised BUNDLE'S NETWORK (TR, 2026-09-15). A stand-alone
        % copy of ..._sfaEI_Sc0p2sig0p1_noise0p025_dualStd_3cond_mu8p25 (SFA on
        % both cell types, per-neuron setpoint S_c_i = 0.2 + 0.1 randn) with
        % three changes:
        %   mu_tilde_relative  8.25 -> 7 on every route (sign by presynaptic
    ...
    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0_dualStd_3cond_mu7revised'
        % Memory-capacity twin of the mu7 noStim preset: Wiener process off
        % (the config names sra1 as the MC integrator). Same bundle
        % (mu7revised_config), chained under the within-bundle rule.
        [d, model_class, conditions] = srnn_param_preset( ...
    ...
    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7revisedMedium'
        % THE mu7revisedMedium BUNDLE'S NETWORK (TR, 2026-09-16): physically identical to mu7revisedAgain (and mu7revised); the bundle exists because its config fixes run_mode = 'medium' (the manuscript-statistics run) with the corrected illustration protocol. A stand-alone
        % copy of ..._sfaEI_Sc0p2sig0p1_noise0p025_dualStd_3cond_mu8p25 (SFA on
        % both cell types, per-neuron setpoint S_c_i = 0.2 + 0.1 randn) with
        % three changes:
        %   mu_tilde_relative  8.25 -> 7 on every route (sign by presynaptic
    ...
    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0_dualStd_3cond_mu7revisedMedium'
        % Memory-capacity twin of the mu7 noStim preset: Wiener process off
        % (the config names sra1 as the MC integrator). Same bundle
        % (mu7revisedMedium_config), chained under the within-bundle rule.
        [d, model_class, conditions] = srnn_param_preset( ...
            'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7revisedMedium');
        d.sigma_u_noise = 0;

    case 'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_steps5s_noise0p025_dualStd_3cond_mu7revisedMedium'
        % Stimulus-staircase twin of the mu7 noStim preset for run_local_lyapunov:
        % a NEW random step every 5 s that never returns to zero -- 8 steps
        % over the stage's revised T = 40 s, no_stim_pattern all false, each
        % step a fresh amp*randn on 20% of each cell type. Same bundle.
    ...
    case 'single_neuron_mu7revisedMedium'
        % Within mu7revisedMedium bundle: reference E neuron without recurrence,
        % noise or heterogeneity, isolating exactly one SFA or STD timescale.
        [d, model_class, ref] = srnn_param_preset( ...
            'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7revisedMedium');
        d.n=1; d.indegree=1; d.n_cellTypes=1; d.cell_type_names={'E'};
...
names = {'default', 'overconnected', ...
    ...
    'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu5', ...
    'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0_dualStd_3cond_mu5', ...
    'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_steps5s_noise0p025_dualStd_3cond_mu5', ...
    'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7', ...
    ...
    'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0p025_dualStd_3cond_mu7revisedMedium', ...
    'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_noStim_noise0_dualStd_3cond_mu7revisedMedium', ...
    'celltype_pairs_sfaEI_Sc0p2sig0p1_tauSpread0p25_steps5s_noise0p025_dualStd_3cond_mu7revisedMedium', ...
    ...
    'single_neuron_dualStd', 'single_neuron_mu7revised', 'single_neuron_mu7revisedAgain', 'single_neuron_mu7revisedMedium', 'mc_pairs_dualStd'};
end
