...
for i = 1:numel(d)
    ...
    % The reference condition the parameter list is quoted "as run in". Resolved
    % rather than named: condition names now state their timescale counts, so
    % the full-adaptation regime is sfa3_std2, sfa3_std1 or sfa1_std1_stf1
    % depending on the preset. This reads SAVED runs, including ones written
    % before the rename, so it falls back to the old literal for those.
    [e.ref_cond_name, e.ref_cond] = pick_full_condition(psa);
...
function [name, cond] = pick_full_condition(psa)
% PICK_FULL_CONDITION The most-adapted condition of a saved run: its name and
% struct, or ('', []) if the run has neither a conforming name nor the legacy one.
%
% Two vocabularies, because this reads run directories of any age: current runs
% name conditions sfaX_stdY[_stfZ], and full_adaptation_condition resolves those;
% runs written before 2026-09-03 use sfa_and_std, which that helper rejects.
name = ''; cond = [];
if isempty(psa.conditions); return; end
try
    name = full_adaptation_condition(psa.conditions);
catch
    name = 'sfa_and_std';          % legacy run
end
...
name = '';                          % neither vocabulary matched
end
...
fprintf(fid, '\n');
fprintf(fid, '%s\n', ['Parameters are listed **as run in the most-adapted condition** (every ' ...
    'adaptation mechanism this preset has, switched on). The other adaptation regimes differ ' ...
    'from it only in the condition-owned fields listed under *Adaptation conditions*, and the ' ...
    'source column names the exact condition each value came from.']);
fprintf(fid, '\n');
end
...
% subset (tau_sensitivity runs the full-adaptation regime alone), so show the
% richest set.
...
if ~isempty(e.ref_cond) && isfield(e.ref_cond, name)
    r.kind = 'condition';
    r.source = sprintf('condition `%s`', e.ref_cond_name);
    r.value = e.ref_cond.(name);
