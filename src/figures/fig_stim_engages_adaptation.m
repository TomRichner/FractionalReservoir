...
%% Run
% Empty condition name -> the preset's most-adapted regime. bursting_pairs has
% single-timescale depression, so that is sfa3_std1, not the sfa3_std2 the
% paper's networks carry -- which is why this is resolved rather than named.
...
if cfg.include_chi2
    % Second output dropped: it was the chi-squared summary text for the
    % per-figure README, which is no longer written. The CALL stays -- it builds
    % fig3, which the sheet needs.
    [fig3, ~] = chi2_panel(t_full, x_E, r_E, dc_levels, ...
