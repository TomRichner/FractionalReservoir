...
% mu_*: RMT block means, indexed (post, pre) -- mu_EI is the block of I
% presynaptic weights onto E. Labelled as routes PRE -> POST (TR, 2026-09-15),
% so mu_EI reads I->E and mu_IE reads E->I. level_of_chaos is 'All Weights'.
...
row_style = containers.Map( ...
    {'f_E', 'level_of_chaos', 'n', ...
     'mu_EE_relative', 'mu_EI_relative', 'mu_IE_relative', 'mu_II_relative'}, ...
    {struct('xlabel', 'E:I neuron ratio', 'xticks', [0.2 0.5 0.8], ...      % n = 500 (see fig_sensitivity_medians)
                                          'xticklabels', {{'100:400','250:250','400:100'}}), ...
     struct('xlabel', 'All Weights',      'xticks', [], 'xticklabels', {{}}), ...
     struct('xlabel', 'Network Size',     'xticks', [100 500 1000], 'xticklabels', {{}}), ...
     struct('xlabel', 'E\rightarrowE',    'xticks', [], 'xticklabels', {{}}), ...
...
% One sheet pair (core / mu) per registry measure with in_sheets set
% (sweep_metrics); the replot above drew every one of them.
specs_all = sweep_metrics();
specs_all = specs_all([specs_all.in_sheets]);
metric_specs = struct( ...
    'name',      {specs_all.field}, ...
    'ylabel',    {specs_all.label}, ...
    'fig_tag',   cellfun(@(s) sprintf('Fig_Sensitivity_%s', s), {specs_all.stem}, 'UniformOutput', false), ...
    'zero_line', {specs_all.zero_line}, ...
    'clim_frac', num2cell(0.8 * ones(1, numel(specs_all))), ...
...
for si = 1:numel(sheets)
    ...
    for a = 1:numel(ax_all)
        ...
        if use_pct
            apply_percent_axis(ax, default_value(this_param), rs.xlabel, label_fs, [-50 0 50]);
        else
