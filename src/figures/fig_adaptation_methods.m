...
% The most-adapted regime, resolved rather than named: this figure runs on two
% different presets (single_neuron_dualStd -> sfa3_std2, single_neuron_stf ->
% sfa1_std1_stf1), which no single literal covers.
full = full_adaptation_condition(conditions);
ci   = find(cellfun(@(c) strcmp(c.name, full), conditions), 1);
